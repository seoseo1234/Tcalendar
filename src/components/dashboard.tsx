...
export default function Dashboard() {
  ...
  return <div className="app-shell">
        ...
        {activeSection === "calendar" ? <div className="workspace">
          ...
          <aside className="right-rail">
            <section className="rail-card quick-add"><h2>빠른 추가</h2>
              ...
              <button className={`quick-photo-input ${analyzing ? "is-analyzing" : ""}`} onClick={() => setModal("photo")} disabled={loading || analyzing}><Images /><span><strong>{analyzing ? "사진 분석 중..." : "사진으로 추가하기"}</strong><small>사진 촬영 또는 갤러리에서 선택</small></span><Plus /></button><button className="manual-add-card" onClick={() => setModal("manual")}><Plus /><span><strong>직접 추가하기</strong><small>날짜와 내용을 직접 작성</small></span><ChevronRight /></button></section>
          </aside>
    ...
    <Modal title="사진으로 추가하기" open={modal === "photo"} onClose={() => setModal(null)}><div className="modal-body"><div className="photo-source-options"><label className="photo-source camera-source"><Camera /><span><strong>사진 촬영</strong><small>카메라로 안내문을 바로 촬영합니다.</small></span><input type="file" accept="image/*" capture="environment" onChange={handleImages} /></label><label className="photo-source gallery-source"><Images /><span><strong>갤러리에서 선택</strong><small>사진을 최대 10장까지 선택합니다.</small></span><input type="file" multiple accept="image/png,image/jpeg,image/webp" onChange={handleImages} /></label></div><p className="upload-help">서로 이어지는 안내문도 최대 10장까지 한 번에 분석할 수 있어요.</p></div></Modal>
